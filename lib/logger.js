...
exports.Logger = class Logger {
   ...
   * @param {mixed} message - the message to be written
  ...
  _getDate() {
    const date = new Date();
    return this._bbriviatedWeekDays[date.getDay()] + ':' +
   ...
   * @returns {mixed} args - the passed args
   */
  _defaultDebug() {}
   ...
   * @param {string} kind - additional info about the operation
   * @param {mixed} message - the message to be written - message to log
  ...
  _log(type, kind, message, color, style) {
    ...
    let output = this._getDate() + ' ' + this._namespace + type + ' - ' + kind;
    if (message) {
      output += ': ' + message;
    }
    ...
    if (!isProd && this._subscribedTypes.indexOf(type) !== -1) {
      /*eslint-disable */
      console.log(colors[style][color]('  ' + output));
      /*eslint-enable */
    }
    return output;
  }
